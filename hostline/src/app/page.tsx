...
const steps = [
  ...
  },
  {
    n: "03",
    title: "Desk acts",
    copy: "Leads land in your board. Bookings and callbacks are logged as they happen.",
  },
];
    ...
    <main className="atmosphere min-h-dvh overflow-x-hidden">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5 md:px-8">
        ...
        <nav className="flex items-center gap-6 text-sm">
          <a href="#path" className="hidden text-[var(--ink)]/70 hover:text-[var(--ink)] md:inline">
            How it works
          </a>
          <Link href="/login" className="cta-start text-sm">
            Start Hostline
          </Link>
        </nav>
      </header>
      ...
      <section className="relative mx-auto grid min-h-[calc(100dvh-5rem)] max-w-6xl items-center gap-10 px-6 pb-16 pt-8 md:grid-cols-[1.05fr_0.95fr] md:px-8 md:pb-20">
        <div className="relative z-10 max-w-xl">
          ...
          <p className="mt-6 max-w-[36ch] text-base leading-relaxed text-[var(--ink)]/75">
            AI receptionist for South African SMEs, powered by Grok Voice Agent. Answers, qualifies, and books while you work.
          </p>
          <div className="mt-8 flex flex-wrap items-center gap-5">
            <Link href="/login" className="cta-start">
              Start Hostline
            </Link>
            <a href="#path" className="cta-see">
              See the desk
            </a>
          </div>
        </div>
      ...
      <section id="path" className="border-t border-[var(--line)] bg-[var(--paper)]">
      ...
      <section className="mx-auto grid max-w-6xl gap-12 px-6 py-20 md:grid-cols-2 md:px-8 md:py-24">
        <div>
          ...
          <p className="mt-5 max-w-[48ch] text-[var(--ink)]/70 leading-relaxed">
            Hostline sits on xAI Voice Agent Builder and the realtime Voice Agent API. One speech-to-speech path, tool calling into your desk, and a phone number you can keep or bring over SIP.
          </p>
          <ul className="mt-8 space-y-3 text-[var(--ink)]/80">
            <li>Free provisioned number or bring your own via SIP</li>
            <li>Tools for leads, bookings, and human handoff</li>
            <li>English, Afrikaans, Zulu, and Xhosa ready prompts</li>
          </ul>
        </div>
        <div className="rounded-[20px] bg-[var(--panel)] p-8 text-[var(--paper)]">
          <p className="mono text-[11px] uppercase tracking-[0.18em] text-white/45">Why Grok only</p>
          <p className="mt-4 text-2xl font-medium tracking-tight leading-snug">
            Telephony, knowledge, tools, and call logs in one voice stack. Less glue. Clearer minute pricing.
          </p>
          <p className="mt-6 text-sm text-white/55">
            About $0.05 per minute of audio on Grok Voice, with optional telephony on a provisioned number.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-20 md:px-8">
        <h2 className="text-3xl font-semibold tracking-tight md:text-4xl">Setup in an afternoon</h2>
        <ol className="mt-10 max-w-2xl space-y-6">
          <li className="grid grid-cols-[auto_1fr] gap-4">
            <span className="mono text-sm text-[var(--mute)]">1</span>
            <p>Create a Grok voice agent in Voice Agent Builder and attach your tools.</p>
          </li>
          <li className="grid grid-cols-[auto_1fr] gap-4">
            <span className="mono text-sm text-[var(--mute)]">2</span>
            <p>Drop your xAI key and agent id into Hostline.</p>
          </li>
          <li className="grid grid-cols-[auto_1fr] gap-4">
            <span className="mono text-sm text-[var(--mute)]">3</span>
            <p>Point tool webhooks at Hostline. Watch leads and calls land on the desk.</p>
          </li>
        </ol>
      </section>
      ...
      <section className="border-t border-[var(--line)] bg-[var(--panel)] text-[var(--paper)]">
        <div className="mx-auto flex max-w-6xl flex-col items-start gap-8 px-6 py-20 md:flex-row md:items-end md:justify-between md:px-8">
          <div>
            <h2 className="max-w-[14ch] text-3xl font-semibold tracking-tight md:text-5xl">
              Put Hostline on your number
            </h2>
            <p className="mt-4 max-w-[40ch] text-white/60">
              Open the desk, wire Grok, and take the next call without a receptionist on shift.
            </p>
          </div>
          <Link href="/login" className="cta-open">
            Open the desk
          </Link>
        </div>
      </section>
...
function Monogram({ small = false }: { small?: boolean }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" aria-hidden>
      ...
      <path
        d="M9 22V10h2.4v4.7H20.4V10H22.8v12h-2.4v-5.1H11.4V22H9z"
        fill="#F7FAFC"
      />
